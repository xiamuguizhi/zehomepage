...
(() => {
    window.ViewImage = new function () {
        ...
        this.listener = (e) => {
            ...
            const images = [...contain.querySelectorAll(_class)].map(_ => _.href || _.getAttribute('zoomSrc') ||  _.src);
            this.display(images, el.href || el.getAttribute('zoomSrc') || el.src);
            e.stopPropagation();
            e.preventDefault();
        };
        this.init = (val) => {
        ...
        this.display = (images, src) => {
                ...
                <div class="view-image">
                    ...
                    <div class="view-image-tools">
                        ...
                        <div class="view-image-tools__flip">
                            <div class="view-image-btn view-image-tools__flip-prev" x-show="${images.length}>1">
                                <svg width="20" height="20" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg"><rect width="48" height="48" fill="white" fill-opacity="0.01"/><path d="M31 36L19 24L31 12" stroke="white" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"/></svg>
                            </div>
                            <button @click="download($refs.viewimg.src);" class="view-image-btn text-sm px-2" style="width: auto;">
                            保存
                            </button>
                            <div class="view-image-btn view-image-tools__flip-next" x-show="${images.length}>1">
            ...
            `, 'text/html').body.firstChild,
                ...
                loadImage = (src) => {
                    ...
                    setTimeout(() => {
                        ...
                        img.onload = function () {
                            setTimeout(() => {
                                $lead.innerHTML = `<img x-ref="viewimg" src="${img.src}" alt="ViewImage" no-view/>`;
            ...
            $el.onclick = (e) => {
                if (e.target.closest('.view-image-close')) {
                    ...
                    if (e.target.closest('.view-image-tools__flip-next')) {
                        index = index === images.length - 1 ? 0 : index + 1;
                    }
                    if (e.target.closest('.view-image-tools__flip-prev')||e.target.closest('.view-image-tools__flip-next')) {
